...
function HomePage() {
  ...
  const handleSubmitPay = async (event) => {
    ...
    const res = await axios.post('http://localhost:3005/pay', {email: email});
  ...
  const handleSubmitSub = async (event) => {
    ...
    if (result.error) {
      ...
      const res = await axios.post('http://localhost:3005/sub', {'payment_method': result.paymentMethod.id, 'email': email});
